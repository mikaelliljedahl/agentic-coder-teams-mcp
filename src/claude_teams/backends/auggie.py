"""Auggie backend integration."""

from typing import ClassVar

from claude_teams.backends.base import BaseBackend, SpawnRequest


class AuggieBackend(BaseBackend):
    """Backend adapter for Augment Code CLI (auggie)."""

    _name = "auggie"
    _binary_name = "auggie"

    _MODEL_MAP: ClassVar[dict[str, str]] = {
        "fast": "claude-haiku-4.5",
        "balanced": "claude-sonnet-4.5",
        "powerful": "claude-opus-4.6",
    }

    def supported_models(self) -> list[str]:
        """Return supported Auggie model names.

        Auggie supports any model via ``--model``; these are the common ones.

        Returns:
            list[str]: Curated list of supported model identifiers.

        """
        return ["claude-haiku-4.5", "claude-sonnet-4.5", "claude-opus-4.6"]

    def default_model(self) -> str:
        """Return the default Auggie model.

        Returns:
            str: Default model identifier for this backend.

        """
        return "claude-sonnet-4.5"

    def resolve_model(self, generic_name: str) -> str:
        """Map a generic or direct model name to an Auggie model.

        Allows pass-through for unrecognized model names.

        Args:
            generic_name (str): Generic tier or direct model name.

        Returns:
            str: Auggie model identifier.

        """
        if generic_name in self._MODEL_MAP:
            return self._MODEL_MAP[generic_name]
        return generic_name

    def build_command(self, request: SpawnRequest) -> list[str]:
        """Build the Auggie CLI command for non-interactive execution.

        Args:
            request (SpawnRequest): Backend-agnostic spawn parameters.

        Returns:
            list[str]: Command parts list.

        """
        binary = self.discover_binary()
        model = self.resolve_model(request.model)
        return [
            binary,
            "-i",
            request.prompt,
            "--model",
            model,
            "--print",
        ]
