...
def test_concurrent_stops_never_lose_an_increment(tmp_path, workers, monkeypatch):
    ...
    def run():
        hooks._record_event(tmp_path, "worker", "Stop", "s1")
    ...
    for thread in threads:
        thread.join(10)
    assert _read(tmp_path)["idle_seq"] == workers
